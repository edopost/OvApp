...
public class DetailedStopList {
	...
	public void update() {
		 ...
		 while(i.hasNext()) {
				...
				System.out.println("1st key : " + me.getKey());
				Iterator iTpc = set.iterator();
				while(iTpc.hasNext()) {
					Map.Entry meTpc = (Map.Entry)iTpc.next();
					System.out.println("2nd key : " + meTpc.getKey());
					...
					while(iCol.hasNext()) {
						...
						System.out.println("3dth key : " + meCol.getKey());
						if(meCol.getValue() instanceof JSONObject) {
							...
							while(iMes.hasNext()) {
								...
								System.out.println("4th key : " + meMes.getKey());
								if(meMes.getKey().equals("Passes")) {
									...
									while(iD.hasNext()) {
										...
										detailedStops.add(new DetailedStop(stopDetailed, isTimingStop, destinationName50, lineDirection, operationDate, timingPointCode, lineName, linePublicNumber, sideCode, timingPointName, tripStopStatus, userStopCode, journeyStopType, targetArrivalTime, targetDepartureTime, expectedArrivalTime));
